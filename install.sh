...
APP_NAME="grit"
APP_VERSION="0.1.0-alpha.1734311808"
ARTIFACT_DOWNLOAD_URL="${INSTALLER_DOWNLOAD_URL:-https://github.com/getgrit/gritql/releases/download/v0.1.0-alpha.1734311808}"
PRINT_VERBOSE=${INSTALLER_PRINT_VERBOSE:-0}
PRINT_QUIET=${INSTALLER_PRINT_QUIET:-0}
NO_MODIFY_PATH=${INSTALLER_NO_MODIFY_PATH:-0}
read -r RECEIPT <<EORECEIPT
{"binaries":["CARGO_DIST_BINS"],"binary_aliases":{},"cdylibs":["CARGO_DIST_DYLIBS"],"cstaticlibs":["CARGO_DIST_STATICLIBS"],"install_prefix":"AXO_INSTALL_PREFIX","provider":{"source":"cargo-dist","version":"0.22.0"},"source":{"app_name":"grit","name":"gritql","owner":"getgrit","release_type":"github"},"version":"0.1.0-alpha.1734311808"}
EORECEIPT
# Are we happy with this same path on Linux and Mac?
RECEIPT_HOME="${HOME}/.config/grit"
...
The installer for grit 0.1.0-alpha.1734311808

This script detects what platform you're on and fetches an appropriate archive from
https://github.com/getgrit/gritql/releases/download/v0.1.0-alpha.1734311808
...
download_binary_and_run_installer() {
    ...
    case "$_artifact_name" in 
        "grit-aarch64-apple-darwin.tar.gz")
            ...
            _updater_bin=""
            ;;
        "grit-aarch64-unknown-linux-gnu.tar.gz")
            ...
            _updater_bin=""
            ;;
        "grit-x86_64-apple-darwin.tar.gz")
            ...
            _updater_bin=""
            ;;
        "grit-x86_64-pc-windows-msvc.tar.gz")
            _arch="x86_64-pc-windows-msvc"
            _zip_ext=".tar.gz"
            _bins="grit.exe"
            _bins_js_array='"grit.exe"'
            _libs=""
            _libs_js_array=""
            _staticlibs=""
            _staticlibs_js_array=""
            ...
            _updater_bin=""
            ;;
        "grit-x86_64-unknown-linux-gnu.tar.gz")
...
select_archive_for_arch() {
    ...
    case "$_true_arch" in 
        "aarch64-apple-darwin")
            _archive="grit-aarch64-apple-darwin.tar.gz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
            _archive="grit-x86_64-apple-darwin.tar.gz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
        ...
        "aarch64-pc-windows-msvc")
            _archive="grit-x86_64-pc-windows-msvc.tar.gz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
            ;;
        "aarch64-unknown-linux-gnu")
            _archive="grit-aarch64-unknown-linux-gnu.tar.gz"
        ...
        "x86_64-pc-windows-msvc")
            ...
            ;;
        "x86_64-unknown-linux-gnu")
            _archive="grit-x86_64-unknown-linux-gnu.tar.gz"
            if ! check_glibc "2" "31"; then
                _archive=""
            fi
            if [ -n "$_archive" ]; then
...
install() {
    ...
    if [ -n "${GRIT_INSTALL_DIR:-}" ]; then
        _force_install_dir="$GRIT_INSTALL_DIR"
    ...
    if [ -z "${_install_dir:-}" ]; then
        ...
        if [ -n "${HOME:-}" ]; then
            ...
            _lib_install_dir="$HOME/.grit/bin"
            _receipt_install_dir="$_install_dir"
